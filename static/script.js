document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendMessage() {
        ...
        if (userMessage.trim() !== '') {
            ...
            if (userMessage && userMessage.startsWith('/generate')){
                const response = await fetch('https://mrlwitwma.pythonanywhere.com/generate', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `user_id=${userId}&user_input=${encodeURIComponent(userMessage)}`
                });
                const responseData = await response.json();
                displayMessage(responseData.response, 'ai', responseData.image); // Allow HTML responses to work
            }
            else{
    ...
    function displayMessage(message, sender, image) {
        ...
        if (sender === 'ai' && message.includes("```")) {
            ...
            if (image) {
                ...
                imageDIV.classList.add(sender);  // Assuming sender is a CSS class name
                imageDIV.setAttribute('src', 'data:image/png;base64,' + image);  // Assuming the message is a base64 encoded PNG image
                messageDiv.appendChild(imageDIV);  // Assuming messageDiv is the container where you want to append the image
            }
        }
